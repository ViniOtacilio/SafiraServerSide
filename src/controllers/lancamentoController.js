...
const createLancamento = async (req, res, next) => {
    ...
    const { value, tipo_de_transacao, categoriaid, titulo_lancamento, data_lancamento, comentario } = req.body;
    const userid = req.user.user_id;

    try {
        await createNewLancamento(value, tipo_de_transacao, userid, categoriaid, titulo_lancamento, data_lancamento, comentario);
        res.sendStatus(201);
        next();
    }
    catch (e) {
